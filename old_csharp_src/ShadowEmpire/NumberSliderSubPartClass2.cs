...
{
  public class NumberSliderSubPartClass2 : SubPartClass
  {
    ...
    public NumberSliderSubPartClass2(
      GameClass tgame,
      string tprefix,
      string tsuffix,
      int twidth,
    ...
    public override Bitmap Paint()
    {
      ...
      if ((double) DrawMod.TGame.Data.RuleVar[839] <= 0.0)
        DrawMod.DrawBlock(ref Expression, 0, 0, this.Width, 20, 0, 0, 0, 128);
      if (DrawMod.TGame.Data.Product == 7)
        DrawMod.DrawBlock(ref Expression, 0, 20, this.Width, 20, 65, 40, 20, (int) byte.MaxValue);
      else
        DrawMod.DrawBlock(ref Expression, 0, 20, this.Width, 20, 50, 70, 125, (int) byte.MaxValue);
      int num1 = this.Width - 91;
      int num2 = (this.maxval <= 0 ? num1 : (int) Math.Round(Conversion.Int((double) this.curval / (double) this.maxval * (double) num1))) - 11;
      if (num2 < 0)
        num2 = 0;
      ...
      DrawMod.DrawSimple(ref local3, ref local4, 20, 20);
    ...
    public override int HandleMouseUp(int x, int y)
    {
      if (!(this.Scroller | this.clickscroll == 1))
        return -1;
      this.curval = (int) Math.Round(Conversion.Int((double) (x - 40) / (double) (this.Width - 91) * (double) this.maxval));
