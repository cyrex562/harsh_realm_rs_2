...
{
  public class BigMiniMapWindowClass : WindowClass
  {
    private int MapId;
    ...
    public BigMiniMapWindowClass(ref GameClass tGame, int tminheight = 0, int tminwidth = 200)
      : base(ref tGame, tGame.ScreenWidth - tminwidth, tGame.ScreenHeight - tminheight)
    {
      this.minheight = tminheight;
      this.minwidth = tminwidth;
      this.hidden = false;
      SubPartClass tsubpart = (SubPartClass) new MiniMapPartClass(tGame, tx: (tGame.ScreenWidth - tminwidth), ty: (tGame.ScreenHeight - tminheight), talsounits: true, trealhex: true);
        ...
        for (int index = 0; index <= subPartCounter; ++index)
        {
          if (x > this.SubPartX[index] & x < this.SubPartX[index] + this.SubPartW[index] && y > this.SubPartY[index] & y < this.SubPartY[index] + this.SubPartH[index] && this.SubPartID[index] == this.MapId)
          {
            ...
            if (b == 2)
            {
              ...
              SubPartClass tsubpart = (SubPartClass) new MiniMapPartClass(DrawMod.TGame, this.hidden, DrawMod.TGame.ScreenWidth - this.minwidth, DrawMod.TGame.ScreenHeight - this.minheight, true, true);
