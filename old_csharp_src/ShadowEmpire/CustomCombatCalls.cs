...
{
  public class CustomCombatCalls
  {
    public SimpleList SL;
    public SimpleList MODL;
    public SimpleList logLeaderBonus;
    public SimpleList logLeaderBonusDef;
    public SimpleList otherBonus;
    public SimpleList otherBonusDef;
    public SimpleList SkillCacheList;
    public int slotModelsAdditive;
    public int slotModelsClean;
    public int slotModels;
    public int slotGameKeys;
    public int slotLeaders;
    public int slotRegimes;
    public int slotZones;
    public int slotUnitFeats;
    public int slotCharSkill;
    public int slotChar;
    public int slotAssets;
    public int slotAssetTypes;
    public int slotBehaviour;
    public int slotSkill;
    public int slotRegimeKey;
    public int slotMM;
    public int slotSkillTypes;
    public int rememberRadPts;
    public bool airEnabled;
    public float SKILLEFFECT;
    public bool EntrBunkerInit;
    public float EntrBunkerMod;
    public float EntrAssetMod;

    public CustomCombatCalls()
    {
      ...
      this.MODL = new SimpleList();
      this.logLeaderBonus = new SimpleList();
      this.logLeaderBonusDef = new SimpleList();
      this.otherBonus = new SimpleList();
      this.otherBonusDef = new SimpleList();
      this.SkillCacheList = new SimpleList();
    ...
    public int IndividualMORMod(ref CombatClass cc, int inr, int morMod, int sfnr = -1)
    {
      int people;
      if (inr > -1)
      {
    ...
    public int IndividualXPMod(ref CombatClass cc, int inr, int xpToBeGiven, int sfnr = -1)
    {
      int people;
      if (inr > -1)
      {
    ...
    public int CachedSkillRoll(GameClass game, int charId, int skillId)
    {
    ...
    public int EntrenchModifier(ref CombatClass cc, int indNr, int curX, int curY, int entr)
    {
      if (cc.IList[indNr].IAttacker == 0 & (int) Math.Round(Conversion.Val(cc.game.Data.Designer)) >= 112)
      {
        if (!this.EntrBunkerInit)
        {
          DataClass data = cc.game.Data;
          string str = "bunkerPoints";
    ...
    public int AIHelpCombatChanger(ref CombatClass cc, int forNr, int againstNr, int tHelpCombat)
    {
    ...
    public void PersonalCombatRoll(
      ref CombatClass cc,
      GameClass game,
      int unr,
      int inr,
      ...
      bool hexLossCheck)
    {
      ...
      if (idValue3_1 > 0)
      {
        ...
        if (integer > 0)
        {
          ...
          string data1 = game.Data.StringListObj[this.slotLeaders].GetData(0, integer, 46);
          ...
          if (DrawMod.RandyNumber.Next(0, num4 * 10) <= 10 & (flag1 | Conversions.ToDouble(data1) > (double) DrawMod.RandyNumber.Next(1, 100)) | unitLossCheck & flag1)
          {
            ...
            if (num5 < num6)
            {
              string data2 = game.Data.StringListObj[this.slotLeaders].GetData(0, integer, 3);
              string data3 = game.Data.StringListObj[this.slotLeaders].GetData(0, integer, 4);
              string characterJobTitle = game.EventRelatedObj.Helper_GetCharacterJobTitle(integer);
              if (inr > -1)
              {
                if (str1.Length > 0)
                  str1 += " ";
      ...
      if (idValue3_2 > 0)
      {
        ...
        if (integer > 0)
        {
          ...
          string data8 = game.Data.StringListObj[this.slotLeaders].GetData(0, integer, 46);
          int row = game.Data.StringListObj[this.slotLeaders].FindRow(0, integer);
          if (DrawMod.RandyNumber.Next(0, num8 * 10) <= 5 & (flag2 | Conversions.ToDouble(data8) > (double) DrawMod.RandyNumber.Next(1, 100)) | unitLossCheck & flag2)
          {
            ...
            if (num9 < num10)
            {
              string data9 = game.Data.StringListObj[this.slotLeaders].GetData(0, integer, 3);
              string data10 = game.Data.StringListObj[this.slotLeaders].GetData(0, integer, 4);
              string characterJobTitle = game.EventRelatedObj.Helper_GetCharacterJobTitle(integer);
              if (inr > -1)
              {
                if (str1.Length > 0)
      ...
      if (hexLossCheck)
      {
        ...
        if (location > -1)
        {
          ...
          if (integer2 > 0)
          {
            ...
            if (num11 < num12)
            {
              string data18 = game.Data.StringListObj[this.slotLeaders].GetData(0, integer2, 3);
              string data19 = game.Data.StringListObj[this.slotLeaders].GetData(0, integer2, 4);
              string characterJobTitle = game.EventRelatedObj.Helper_GetCharacterJobTitle(integer2);
              if (str1.Length > 0)
                str1 += " ";
              string str14 = characterJobTitle + " " + data18 + " " + data19 + " was killed in action during the loss of " + game.Data.LocObj[location].Name + ".";
              str1 += str14;
          ...
          if (locationById == location)
          {
            ...
            for (int index25 = 0; index25 <= length; ++index25)
            {
              if ((int) Math.Round(Conversion.Val(game.Data.StringListObj[this.slotLeaders].Data[index25, 5])) == num1)
              {
                ...
                if (num13 == 5 | num13 == 6)
                {
                  ...
                  if (num15 < num16)
                  {
                    string data23 = game.Data.StringListObj[this.slotLeaders].GetData(0, num14, 3);
                    string data24 = game.Data.StringListObj[this.slotLeaders].GetData(0, num14, 4);
                    string characterJobTitle = game.EventRelatedObj.Helper_GetCharacterJobTitle(num14);
                    if (str1.Length > 0)
                      str1 += " ";
                    string str18 = characterJobTitle + " " + data23 + " " + data24 + " was killed in action during the loss of the capital city (" + game.Data.LocObj[location].Name + ").";
                    str1 += str18;
      ...
      if (hexLossCheck)
      {
        DataClass data = game.Data;
        string str22 = "Zones";
        ref string local = ref str22;
        int libVar = data.FindLibVar(ref local, "SE_Data");
        int hexLibVarValue = game.Data.MapObj[0].HexObj[cc.TargetX, cc.TargetY].GetHexLibVarValue(libVar);
        string texty = game.EventRelatedObj.CheckHexName(cc.TargetX, cc.TargetY, 0, 0) + " was conquered by " + game.Data.RegimeObj[cc.AttackerRegime].Name + ". " + str1;
      ...
      else
      {
        ...
        else
        {
          ...
          DataClass data = game.Data;
          string str24 = "Zones";
          ref string local = ref str24;
          int libVar = data.FindLibVar(ref local, "SE_Data");
          int hexLibVarValue = game.Data.MapObj[0].HexObj[cc.TargetX, cc.TargetY].GetHexLibVarValue(libVar);
          string texty = "Attackers from " + game.Data.RegimeObj[cc.AttackerRegime].Name + " lost a Leader: " + str1;
    ...
    public void PreCombatCall(ref CombatClass cc, GameClass game)
    {
      ...
      for (int index1 = 0; index1 <= ucounter2; ++index1)
      {
        ...
        if (simpleList1.Counter > -1)
        {
          ...
          {
            int icounter = cc.ICounter;
            for (int tid = 0; tid <= icounter; ++tid)
            {
              if (Information.IsNothing((object) cc.IList[tid].IunitFeat))
                cc.IList[tid].IunitFeat = new SimpleList();
            ...
            for (int index5 = 1; index5 <= num5; ++index5)
            {
              ...
              if (randomIdbasedOnWeight > -1)
              {
                if (Information.IsNothing((object) cc.IList[randomIdbasedOnWeight].IunitFeat))
                  cc.IList[randomIdbasedOnWeight].IunitFeat = new SimpleList();
          ...
          for (int index6 = 0; index6 <= counter3; ++index6)
          {
            ...
            int num7 = index7;
            for (int index8 = 1; index8 <= num7; ++index8)
            {
              if (simpleList1.Data2[index6] >= 2)
              {
                int num8 = simpleList1.Data2[index6];
                for (int index9 = 2; index9 <= num8; ++index9)
                {
    ...
    public void DoStructuralDamageCall(ref CombatClass cc, GameClass game, int damPts)
    {
      int num1 = (int) Math.Round((double) cc.CombatRound / 3.0) + 1;
      string s = "";
      DataClass data1 = game.Data;
      string str1 = "Zones";
      ...
      if (damPts < 1)
        return;
      DataClass data2 = DrawMod.TGame.Data;
      string str2 = "bunkerPoints";
      ...
      if (damPts > 0)
      {
        if (cc.CombatType == 5)
        {
          DataClass data3 = game.Data;
          string str3 = "aiStrucDam";
        ...
        if (simpleList.Counter > -1)
        {
          ...
          for (int index = 0; index <= counter2; ++index)
          {
            ...
            if (row1 > -1)
            {
              ...
              string str4 = game.Data.StringListObj[this.slotAssetTypes].GetData(0, idValue1, 1);
              if (nr > 0)
      ...
      cc.AddBiggy(s);
      string str5 = s.Replace(". ", ".\r\n");
      string texty1 = game.EventRelatedObj.CheckHexName(cc.TargetX, cc.TargetY, 0, 0) + "(" + cc.TargetX.ToString() + "," + cc.TargetY.ToString() + ") was attacked.\r\n" + str5;
    ...
    public void EndCombatCall(ref CombatClass cc, GameClass game)
    {
      int num1 = (int) Math.Round((double) cc.CombatRound / 3.0) + 1;
      SimpleList simpleList1 = new SimpleList();
      ...
      for (int index1 = 0; index1 <= counter2; ++index1)
      {
        ...
        for (int index2 = 1; index2 <= combatRound; ++index2)
        {
          if (index2 <= 10)
    ...
    public void EndBattleCall(ref CombatClass cc, GameClass game)
    {
      ...
      int ucounter1 = cc.UCounter;
      for (int index7 = 0; index7 <= ucounter1; ++index7)
      {
      ...
      if (this.slotUnitFeats > -1)
      {
        ...
        int ucounter2 = cc.UCounter;
        for (int index10 = 0; index10 <= ucounter2; ++index10)
        {
          int unr = cc.UList[index10].UNr;
          int historical = game.Data.UnitObj[unr].Historical;
          int icounter = cc.ICounter;
          for (int index11 = 0; index11 <= icounter; ++index11)
          {
      ...
      for (int index12 = 0; index12 <= ucounter3; ++index12)
    ...
    public void StartCombatRound(ref CombatClass cc, GameClass game, int combatRound)
    {
      int num1 = -1;
      ...
      int num8;
      int num9;
      for (int index1 = 0; index1 <= icounter1; ++index1)
      {
      ...
      int index9;
      for (int index10 = 0; index10 <= icounter2; ++index10)
      {
        if (Information.IsNothing((object) cc.IList[index10].IunitFeat))
          cc.IList[index10].IunitFeat = new SimpleList();
        string txt = "";
            ...
            if (cc.IList[cc.FindISlot(cc.IList[index10].IunitFeat.Data1[index11])].IKilled == 0 & cc.IList[cc.FindISlot(cc.IList[index10].IunitFeat.Data1[index11])].IRetreated == 0 & cc.IList[cc.FindISlot(cc.IList[index10].IunitFeat.Data1[index11])].IRetreat == 0)
            {
              ...
              for (int index12 = 0; index12 <= length; ++index12)
              {
                ...
                if ((int) Math.Round(Conversion.Val(game.Data.StringListObj[this.slotBehaviour].Data[index12, 1])) == idValue)
                {
                  ...
                  int index13;
                  string str1 = game.Data.StringListObj[this.slotBehaviour].Data[index12, index13];
                  ...
                  if (flag1)
                  {
                    ...
                    {
                      string str2 = data + " gives " + num16.ToString() + "% chance on a breakthrough. ";
                      if (DrawMod.RandyNumber.Next(0, 100) < num16)
                      {
                        txt = str2 + "Succes!";
                    ...
                    if (num11 == 29 & cc.UList[cc.IList[index10].IUlistNr].URetreat < 1)
                    {
                      ...
                      for (int tid = 0; tid <= icounter3; ++tid)
                      {
                      ...
                      if (simpleList2.Counter > -1 & simpleList3.Counter > -1)
                      {
                        ...
                        int num21 = num17;
                        for (int index20 = 1; index20 <= num21; ++index20)
                        {
    ...
    public int NumberOfMods() => this.airEnabled ? 22 : 15;

    public string GetModName(int nr)
    {
      switch (nr)
      {
        case 1:
    ...
    public int IndividualCombatCall_HasNoEarlyCombatRoundPenalties(
      ref CombatClass cc,
      GameClass game,
      int attnr,
      int defnr,
        ...
        if (!Information.IsNothing((object) cc.IList[index2].IunitFeat))
        {
          ...
          for (int index4 = 0; index4 <= counter; ++index4)
          {
            if (cc.IList[index2].IunitFeat.Id[index4] > 0 && cc.IList[cc.FindISlot(cc.IList[index2].IunitFeat.Data1[index4])].IKilled == 0 & cc.IList[cc.FindISlot(cc.IList[index2].IunitFeat.Data1[index4])].IRetreated == 0 & cc.IList[cc.FindISlot(cc.IList[index2].IunitFeat.Data1[index4])].IRetreat == 0)
            {
              ...
              string data = game.Data.StringListObj[this.slotUnitFeats].GetData(0, idValue, 2);
              int length = game.Data.StringListObj[this.slotBehaviour].Length;
              for (int index5 = 0; index5 <= length; ++index5)
              {
                ...
                if ((int) Math.Round(Conversion.Val(game.Data.StringListObj[this.slotBehaviour].Data[index5, 1])) == idValue)
                {
                  ...
                  int index6;
                  string str = game.Data.StringListObj[this.slotBehaviour].Data[index5, index6];
    ...
    public int UnitCombatCall_AvoidPanic(GameClass game, int unr, ref string s9)
    {
      int num = -1;
      int idValue2 = -1;
    ...
    public int UnitAirBridgeBonus(GameClass game, int unr)
    {
      int num1 = -1;
      int idValue2 = -1;
      int historical1 = game.Data.UnitObj[unr].Historical;
    ...
    public int IndividualCombatCall_RiverTypeAndLandscapeTypeModifier(
      ref CombatClass cc,
      GameClass game,
      int attnr,
      int defnr,
            ...
            if (cc.IList[index1].IunitFeat.Id[index3] > 0 && cc.IList[cc.FindISlot(cc.IList[index1].IunitFeat.Data1[index3])].IKilled == 0 & cc.IList[cc.FindISlot(cc.IList[index1].IunitFeat.Data1[index3])].IRetreated == 0 & cc.IList[cc.FindISlot(cc.IList[index1].IunitFeat.Data1[index3])].IRetreat == 0)
            {
              ...
              for (int index4 = 0; index4 <= length; ++index4)
              {
                ...
                if ((int) Math.Round(Conversion.Val(game.Data.StringListObj[this.slotBehaviour].Data[index4, 1])) == idValue)
                {
                  ...
                  int index5;
                  string str1 = game.Data.StringListObj[this.slotBehaviour].Data[index4, index5];
                  ...
                  if (flag)
                  {
                    ...
                    if (num4 == 35 && index1 == defnr && num9 == -1 | riverType == num9)
                    {
                      ...
                      if (num2 < 100)
                      {
                        ...
                        num13 = num15 - 100;
                        string str5 = num13.ToString();
                        strArray5[2] = str5;
                        strArray4[3] = "% to ";
                        string[] strArray6 = strArray4;
                        num12 = num2 - 100;
                        string str6 = num12.ToString();
                        strArray6[4] = str6;
                        strArray4[5] = "%.";
    ...
    public bool IndividualCombatCall_SmallSizeBGmodifierApplies(
      ref CombatClass cc,
      GameClass game,
      int attnr,
      int defnr)
    ...
    public int IndividualCombatCall_ResultModifier(
      int result,
      ref CombatClass cc,
      GameClass game,
      int attnr,
            ...
            if (cc.IList[index1].IunitFeat.Id[index3] > 0 && cc.IList[cc.FindISlot(cc.IList[index1].IunitFeat.Data1[index3])].IKilled == 0 & cc.IList[cc.FindISlot(cc.IList[index1].IunitFeat.Data1[index3])].IRetreated == 0 & cc.IList[cc.FindISlot(cc.IList[index1].IunitFeat.Data1[index3])].IRetreat == 0)
            {
              ...
              for (int index4 = 0; index4 <= length; ++index4)
              {
                ...
                if ((int) Math.Round(Conversion.Val(game.Data.StringListObj[this.slotBehaviour].Data[index4, 1])) == idValue)
                {
                  ...
                  int index5;
                  string idValue2 = game.Data.StringListObj[this.slotBehaviour].Data[index4, index5];
    ...
    public float IndividualCombatCall_AttValModder(
      int modNr,
      ref CombatClass cc,
      GameClass game,
      int attnr,
            ...
            if (cc.IList[index2].IunitFeat.Id[index4] > 0 && cc.IList[cc.FindISlot(cc.IList[index2].IunitFeat.Data1[index4])].IKilled == 0 & cc.IList[cc.FindISlot(cc.IList[index2].IunitFeat.Data1[index4])].IRetreated == 0 & cc.IList[cc.FindISlot(cc.IList[index2].IunitFeat.Data1[index4])].IRetreat == 0)
            {
              ...
              for (int index5 = 0; index5 <= length; ++index5)
              {
                ...
                if ((int) Math.Round(Conversion.Val(game.Data.StringListObj[this.slotBehaviour].Data[index5, 1])) == idValue)
                {
                  ...
                  int index6;
                  string str1 = game.Data.StringListObj[this.slotBehaviour].Data[index5, index6];
                  ...
                  if (flag2)
                  {
                    ...
                    {
                      string str2 = num16 <= 0 ? "on all skills." : "on skill '" + game.Data.StringListObj[this.slotSkillTypes].GetData(0, num16, 1) + "'";
    ...
    public string GetUnitFeatName(GameClass game, int id)
    {
      string unitFeatName = game.Data.StringListObj[this.slotUnitFeats].GetData(0, id, 2);
      if (unitFeatName.Length < 1)
        unitFeatName = "Unknown Unit Feat";
      return unitFeatName;
    }

    public float IndividualCombatCall_DefValModder(
      int modNr,
      ref CombatClass cc,
      GameClass game,
      int attnr2,
        ...
        if (!Information.IsNothing((object) cc.IList[index4].IunitFeat))
        {
          ...
          for (int index6 = 0; index6 <= counter; ++index6)
          {
            if (cc.IList[index4].IunitFeat.Id[index6] > 0 && cc.IList[cc.FindISlot(cc.IList[index4].IunitFeat.Data1[index6])].IKilled == 0 & cc.IList[cc.FindISlot(cc.IList[index4].IunitFeat.Data1[index6])].IRetreated == 0 & cc.IList[cc.FindISlot(cc.IList[index4].IunitFeat.Data1[index6])].IRetreat == 0)
            {
              ...
              string data = game.Data.StringListObj[this.slotUnitFeats].GetData(0, idValue1, 2);
              int length = game.Data.StringListObj[this.slotBehaviour].Length;
              for (int index7 = 0; index7 <= length; ++index7)
              {
                ...
                if ((int) Math.Round(Conversion.Val(game.Data.StringListObj[this.slotBehaviour].Data[index7, 1])) == idValue1)
                {
                  ...
                  int index8;
                  string str1 = game.Data.StringListObj[this.slotBehaviour].Data[index7, index8];
                  ...
                  if (flag2)
                  {
                    ...
                    {
                      string str2 = idValue2 <= 0 ? "on all skills." : "on skill '" + game.Data.StringListObj[this.slotSkillTypes].GetData(0, idValue2, 1) + "'";
    ...
    public void UnitLost(ref CombatClass cc, GameClass game, int unr) => this.PersonalCombatRoll(ref cc, game, unr, -1, 75, "", true, false);

    public int IndividualCombatCall_FirstAttackOfRound(
      ref CombatClass cc,
      GameClass game,
      int attnr,
      int defnr,
    ...
    public int AlterCombatLastRound(ref CombatClass cc) => cc.CombatType == 13 ? 3 : -1;
  }
}
