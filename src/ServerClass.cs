...
namespace WindowsApplication1
{
  public class ServerClass
  {
    ...
    private void ReadResponse(IAsyncResult ar)
    {
      ...
      {
        asyncState.lastRead = DateAndTime.Now;
        int count = respStream.EndRead(ar);
        if (count > 0 & !asyncState.completed)
        {
          ...
          if (this.e.ServerCommand == ServerCommandType.PlayTurn)
          {
            ...
            for (int index = 0; index <= num; ++index)
              asyncState.upload[(int) (asyncState.uploaddone + (long) index)] = asyncState.buf[index];
    ...
    public void Go()
    {
      this.e.ServerMessages = new MessageList();
      ...
      EditClass e = this.e;
      int serverCommandMaxStep = this.e.ServerCommandMaxStep;
      for (e.ServerCommandStep = 1; this.e.ServerCommandStep <= serverCommandMaxStep; ++this.e.ServerCommandStep)
      {
        if (this.e.ServerCommandStep <= this.e.ServerCommandMaxStep)
        {
          ...
          {
            now = DateTime.Now;
            string str = "----------------------------" + now.Ticks.ToString("x");
            this.wr.Method = "POST";
          ...
          if (!this.e.ServerOrderCancel)
          {
            ...
            if (this.e.ServerCommand == ServerCommandType.PlayTurn & this.e.ServerCommandStep == 1)
            {
              ...
              {
                this.receive = Encoding.ASCII.GetString(state.upload, 0, (int) state.uploaddone);
              }
              else
              {
                this.e.ServerDownloadFile = new byte[(int) (state.uploaddone - 1L) + 1];
                int num = (int) (state.uploaddone - 1L);
                for (int index = 0; index <= num; ++index)
    ...
    private void ScanTimeoutCallback(ref object state, bool timedOut)
    {
      if (!timedOut)
        return;
    ...
    public void ReadReceive()
    {
      ...
      else
      {
        ...
        int num1 = !(Strings.AscW(Strings.Left(str, 1)) < 48 | Strings.AscW(Strings.Left(str, 1)) > 57) ? (int) Math.Round(Conversion.Val(Strings.Left(str, 2))) : 900;
        if (num1 == 99)
          num1 = 900;
        ...
        if (this.e.ServerCommand == ServerCommandType.Refresh)
        {
          if (this.e.ServerCommandStep == 1)
          {
            if (num1 == 0)
            {
              ...
              {
                this.e.ServerChallengeList[index - 1] = new ChallengeClass();
            ...
            if (num1 == 0)
            {
              ...
              try
              {
                ...
                for (int index = 1; index <= num3; ++index)
                {
                  ...
                  this.e.ServerRunningGameList[index - 1] = new RunningGameClass();
    ...
    private class WebRequestState
    {
      ...
      public string tempreceive;
      public byte[] upload;
      public long uploaddone;
      public EditClass e;
