...
{
  pub class SimpleTroopTypeScreenClass : ScreenClass
  {
     WEditMenu: i32;
     Wmiddle: i32;
     wdown: i32;
    ...
    pub SimpleTroopTypeScreenClass( tgame: GameClass, tformref: Form1)
      : base( tgame, tgame.BACKGROUND3MARC, tformref)
    {
      self.OffSetX =  Math.Round( (self.Game.ScreenWidth - 1024) / 2.0);
      self.AllowRightMouse = true;
      self.Game.EditObj.SimpleEditWindow = 103;
      self.Wmiddle = self.AddWindow((WindowClass) new SimpleTroopTypeWindowClass( tgame), 0, 0, self.Game.ScreenWidth, self.Game.ScreenHeight);
    }
