...
{
  public class BigMessageWindowClass : WindowClass
  {
    private int okid;
    private int ok2id;
    private int tbacknr;
    ...
    private int coreMessage;
    private DynamicData dyn;

    public BigMessageWindowClass(ref GameClass tGame)
    ...
    public void ViewMessage()
    {
      ...
      if (this.game.Data.RegimeObj[this.game.Data.Turn].MesChosen[this.coreMessage] > 0)
      {
        ...
        for (int tdata = 0; tdata <= elementCounter; ++tdata)
        {
      ...
      if (this.OptionsList5Obj.ListCount > -1)
      {
        ...
        {
          SubPartClass tsubpart = (SubPartClass) new BigListSubPartClass(this.OptionsList5Obj, 4, 565, tlistselect1, this.game, tHeaderCenter: false, tdotopandbottom: false, tbackbitmap: (ref this.OwnBitmap), bbx: 50, bby: 550, tMarcStyle: true);
          this.OptionsList5id = this.AddSubPart(ref tsubpart, 50, 550, 565, 160, 0);
        }
      }
      SubPartClass tsubpart1 = (SubPartClass) new TextButtonPartClass("CLOSE", 150, "Click to close this message popup.", ref this.OwnBitmap, 680, 400, theight: 30, usefont: this.game.MarcFont4, useshadow: true, tMarcStyle: true);
      this.okid = this.AddSubPart(ref tsubpart1, 680, 400, 150, 30, 1);
      SubPartClass tsubpart2 = (SubPartClass) new MarcButtonPartClass(this.game.BACKBUTTON, tDescript: "Exit this message popup [ESC]", tBackbitmap: (ref this.OwnBitmap), bbx: 894, bby: 24);
      ...
      {
        SubPartClass tsubpart3 = (SubPartClass) new DynamicArea(this.game, 565, 510 + num4, this.game.Data.RegimeObj[this.game.Data.Turn].MessString[this.FromMessage], ref this.OwnBitmap, 50, 20);
        this.TAid = this.AddSubPart(ref tsubpart3, 50, 20, 565, 510 + num4, 0);
      }
      else
      {
        SubPartClass tsubpart4 = (SubPartClass) new TextAreaClass2(this.game, 565, (int) Math.Round(Conversion.Int((double) (510 + num4 - 17) / 17.0)), this.game.MarcFont8, "[tab]Message," + this.game.Data.RegimeObj[this.game.Data.Turn].MessString[this.FromMessage] + "[/tab]", 17, ref this.OwnBitmap, 50, 20, tUseEncy: true);
      ...
      if (this.OptionsList5Obj.ListCount > -1)
      {
        if (this.game.Data.RegimeObj[this.game.Data.Turn].MesChosen[this.coreMessage] > 0)
        {
          SubPartClass tsubpart5 = (SubPartClass) new TextButtonPartClass("DECIDE (" + this.dyn.element[this.detailnr].optionpp.ToString() + "PP)", 250, "You have already made a decision.", ref this.OwnBitmap, 640, 605, true, theight: 60, usefont: this.game.MarcFont3, useshadow: true, tMarcStyle: true);
        ...
        {
          SubPartClass tsubpart6 = (SubPartClass) new TextButtonPartClass("DECIDE (" + this.dyn.element[this.detailnr].optionpp.ToString() + "PP)", 250, "Click to make the selected decision.", ref this.OwnBitmap, 640, 605, theight: 60, usefont: this.game.MarcFont3, useshadow: true, tMarcStyle: true);
          this.orderid = this.AddSubPart(ref tsubpart6, 640, 605, 250, 60, 1);
        }
        else
        {
          SubPartClass tsubpart7 = (SubPartClass) new TextButtonPartClass("DECIDE (" + this.dyn.element[this.detailnr].optionpp.ToString() + "PP)", 250, "You do not have the PP needed to take this decision.", ref this.OwnBitmap, 640, 605, true, theight: 60, usefont: this.game.MarcFont3, useshadow: true, tMarcStyle: true);
          this.orderidb = this.AddSubPart(ref tsubpart7, 640, 605, 250, 60, 0);
        }
      }
      else
      {
        SubPartClass tsubpart8 = (SubPartClass) new TextButtonPartClass("DECIDE", 250, "No decision available with report you are viewing right now.", ref this.OwnBitmap, 640, 605, true, theight: 60, usefont: this.game.MarcFont3, useshadow: true, tMarcStyle: true);
      ...
      for (int tdata = 0; tdata <= messCounter; ++tdata)
      {
        if (this.game.Data.RegimeObj[this.game.Data.Turn].MesGroup[tdata] > -1 && this.game.Data.RegimeObj[this.game.Data.Turn].MesGroup[tdata] == this.game.Data.RegimeObj[this.game.Data.Turn].MesGroup[this.coreMessage])
        {
          ...
          if (this.detailnr2 == tdata)
            tlistselect2 = num5;
          string tname = this.game.Data.RegimeObj[this.game.Data.Turn].MesName[tdata].Length <= 1 ? this.game.Data.RegimeObj[this.game.Data.Turn].MessString[tdata] : this.game.Data.RegimeObj[this.game.Data.Turn].MesName[tdata];
          bool flag = false;
      ...
      if (this.OptionsList6Obj.ListCount > 0)
      {
        ...
        else
        {
          ...
          SubPartClass tsubpart9 = (SubPartClass) new ListSubPartClass(optionsList6Obj, 9, 180, tlistselect3, game, tHeaderCenter: false, tdotopandbottom: false, tbackbitmap: (ref local1), bbx: 680, bby: 200, tMarcStyle: true, overruleFont: (ref local2));
      ...
      string str = Strings.Trim(Conversion.Str((object) Math.Round(new Decimal(this.game.Data.RegimeObj[this.game.Data.Turn].ResPts))));
