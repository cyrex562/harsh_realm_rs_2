...
{
  public class MapWindowClass2 : WindowClass
  {
    private int MapId;
    ...
    private DateTime lastclickleft;
    private DateTime lastscroll;
    private int lastMouseOverX;
    private int lastMouseOverY;
    private int lastUnitSelected;

    public MapWindowClass2(ref GameClass tGame, int tminheight = 0, int tminwidth = 200, int tZoomLevel = -2)
    ...
    public override WindowReturnClass handleTimer()
    {
      ...
      if (!this.game.AIThreadRunning && this.game.EditObj.BattleTimerActive)
      {
        if (this.game.EditObj.BattleAnimNr == 0 && this.game.EditObj.udsUnitOrderMode != 33 & this.game.EditObj.OrderType != 33)
        {
          ...
          this.game.EditObj.Volume2 = (int) Math.Round((double) this.game.EditObj.Volume2 / 2.0);
    ...
    public override WindowReturnClass HandleKeyPress(int nr, bool fromTimer = false)
    {
      ...
      int num3 = (int) Math.Round(Conversion.Int((double) (this.OwnBitmap.Height - num1) / (double) (24 * (this.game.EditObj.Zoom + 2))));
    ...
    public object UdsClickUnit(int tx, int ty, int tmap, bool outOfWindowCall)
    {
      WindowReturnClass windowReturnClass = new WindowReturnClass();
      bool flag1 = false;
      bool flag2 = false;
        ...
        if (flag1)
        {
          ...
          if (!Information.IsNothing((object) this.game.EditObj.TempCoordListLastMove))
          {
            if (this.game.EditObj.TempCoordListLastMove.counter == -1)
              ...
              this.game.EditObj.TempCoordList.AddList(ref this.game.EditObj.TempCoordListLastMove);
          }
          if (!outOfWindowCall)
            this.SubPartList[0].Paint();
      ...
      else if (this.game.EditObj.udsUnitOrderMode == 36)
      {
        ...
        this.game.EventRelatedObj.DoCheckSpecificEvent((int) Math.Round(Conversion.Val((object) this.game.Data.RuleVar[702])));
        ...
        this.game.EditObj.TempMovePathList = new CoordList();
        this.game.EditObj.mouseOverActive = false;
        if (!outOfWindowCall)
          this.SubPartList[0].Paint();
        this.game.EditObj.TempCoordListLastMove = coordList;
      ...
      else if (this.game.EditObj.udsUnitOrderMode == 18)
      {
        if (flag2)
        {
          ...
          {
            int integer = Conversions.ToInteger(this.game.Data.StringListObj[this.game.HandyFunctionsObj.GetStringListByID((int) Math.Round((double) this.game.Data.RuleVar[404]))].GetData(0, this.game.Data.UnitObj[this.game.EditObj.UnitSelected].items.list.Id[index], 3));
      ...
      else if (this.game.EditObj.udsUnitOrderMode == 14 | this.game.EditObj.udsUnitOrderMode == 33 | this.game.EditObj.udsUnitOrderMode == 55)
      {
        if (flag4)
        {
          ...
          if (this.game.EditObj.udsUnitOrderMode == 55)
          {
            ...
            if (num2 > 100)
              num2 = 100;
            num1 = (int) Math.Round(Math.Floor((double) (num1 * num2) / 100.0));
          }
          this.game.HandyFunctionsObj.RedimTempValue(9999);
                ...
                if (index3 >= 0 & index4 >= 0 & index3 <= this.game.Data.MapObj[0].MapWidth & index4 <= this.game.Data.MapObj[0].MapHeight && this.game.EditObj.udsUnitOrderMode == 33 | this.game.Data.MapObj[0].HexObj[index3, index4].Regime > -1 & this.game.Data.MapObj[0].HexObj[index3, index4].MaxRecon > 0)
                {
                  ...
                  if (flag6)
                  {
                    ...
                    if (num11 <= num1)
                    {
                      ...
                      Coordinate tcoord;
                      tcoord.y = 0;
                      this.game.HandyFunctionsObj.MovementSpecialCoordSet(this.game.Data.Turn, index3, index4, ref tcoord, this.game.EditObj.udsUnitOrderMode == 33);
      ...
      else if (this.game.EditObj.udsUnitOrderMode == 11)
      {
        if (flag3)
        {
          ...
          int num16 = num12;
          int num17 = num13;
          for (int index5 = num16; index5 <= num17; ++index5)
          {
            int num18 = num14;
            int num19 = num15;
            for (int index6 = num18; index6 <= num19; ++index6)
            {
              ...
              if (index7 >= 0 & index8 >= 0 & index7 <= this.game.Data.MapObj[0].MapWidth & index8 <= this.game.Data.MapObj[0].MapHeight && this.game.Data.MapObj[0].HexObj[index7, index8].Regime > -1 & this.game.Data.MapObj[0].HexObj[index7, index8].MaxRecon > 0 && this.game.EventRelatedObj.Helper_CanAttack(this.game.Data.Turn, this.game.Data.MapObj[0].HexObj[index7, index8].Regime, false) && this.game.Data.MapObj[0].HexObj[index7, index8].Regime != this.game.Data.Turn & this.game.Data.MapObj[0].HexObj[index7, index8].Regime > -1)
              {
                ...
                if (num20 <= maxArtRange)
                {
                  ...
                  Coordinate tcoord;
                  tcoord.y = 0;
                  this.game.HandyFunctionsObj.MovementSpecialCoordSet(this.game.Data.Turn, index7, index8, ref tcoord, false);
          ...
          if (x > this.SubPartX[index1] & x < this.SubPartX[index1] + this.SubPartW[index1] && y > this.SubPartY[index1] & y < this.SubPartY[index1] + this.SubPartH[index1] && this.SubPartID[index1] == this.MapId)
          {
            ...
            if (coordinate1.onmap)
            {
              ...
              if (this.game.SelectX > -1 & this.game.SelectY > -1 & this.game.SelectX <= this.game.Data.MapObj[this.game.EditObj.MapSelected].MapWidth & this.game.SelectY <= this.game.Data.MapObj[this.game.EditObj.MapSelected].MapHeight)
              {
                ...
                bitmap = (Bitmap) null;
                ref Bitmap local = ref bitmap;
                subPart.PaintCoordinate((Graphics) null, selectX, selectY2, map, counterAlpha, ref local);
              }
              Coordinate Target;
              if ((double) this.game.Data.RuleVar[701] > 0.0)
              {
                if (b == 1)
                {
                  if (this.game.EditObj.udsUnitOrderMode == 54)
                  {
                    ...
                    if (hexLibVarValue > 0)
                    {
                      ...
                      num2 = (int) Math.Round(Conversion.Val(this.game.Data.StringListObj[stringListById].GetData(0, hexLibVarValue, 8)));
                      int id = (int) Math.Round(Conversion.Val(this.game.Data.StringListObj[stringListById].GetData(0, hexLibVarValue, 6)));
                      if (id > 0)
                ...
                if (b == 2)
                {
                  if (this.game.EditObj.udsUnitOrderMode == 54)
                  {
                    ...
                    string str = "Zones";
                    ref string local = ref str;
                    this.game.EditObj.OrderSubType = this.game.Data.MapObj[0].HexObj[this.game.SelectX, this.game.SelectY].GetHexLibVarValue(data.FindLibVar(ref local, "SE_Data"));
                    this.DrawLayersAndSuch();
                    WindowReturnClass windowReturnClass2 = (WindowReturnClass) this.UdsClickUnit(this.game.SelectX, this.game.SelectY, 0, false);
                  ...
                  if (this.game.EditObj.OrderType == 11 | this.game.EditObj.OrderType == 14 | this.game.EditObj.OrderType == 33)
                  {
                    ...
                    WindowReturnClass windowReturnClass3 = (WindowReturnClass) this.UdsClickUnit(this.game.SelectX, this.game.SelectY, mapSelected1, false);
                  ...
                  if (this.game.EditObj.OrderType == 55 & this.game.EditObj.TempValue[0].Value[this.game.SelectX, this.game.SelectY] < 999)
                  {
                    ...
                    windowReturnClass1.SetFlag(true);
                    this.game.EditObj.UnitSelected = this.game.EditObj.OrderUnit;
                    WindowReturnClass windowReturnClass4 = (WindowReturnClass) this.UdsClickUnit(this.game.SelectX, this.game.SelectY, mapSelected1, false);
                  ...
                  if (this.game.EditObj.OrderType == 18)
                  {
                    if (this.game.EditObj.TempValue[0].Value[this.game.SelectX, this.game.SelectY] >= this.game.HandyFunctionsObj.GetUnitWeight(this.game.EditObj.OrderUnit, includeLisWeight: true))
                    {
                      ...
                      if (!this.game.HandyFunctionsObj.VisibleEnemyUnitsInHex(this.game.SelectX, this.game.SelectY, this.game.EditObj.MapSelected, this.game.Data.Turn, true))
                      {
                        ...
                        {
                          string infostring = "Air Landing: " + this.game.Data.UnitObj[orderUnit].Name + " disembarks...";
                        ...
                        this.DrawLayersAndSuch();
                        this.game.EditObj.UnitSelected = this.game.EditObj.OrderUnit;
                        WindowReturnClass windowReturnClass5 = (WindowReturnClass) this.UdsClickUnit(this.game.SelectX, this.game.SelectY, mapSelected1, false);
                    ...
                    else
                    {
                      ...
                      windowReturnClass1.SetFlag(true);
                      this.game.EditObj.UnitSelected = this.game.EditObj.OrderUnit;
                      WindowReturnClass windowReturnClass6 = (WindowReturnClass) this.UdsClickUnit(this.game.SelectX, this.game.SelectY, mapSelected1, false);
                  ...
                  if (this.game.EditObj.OrderType == 36)
                  {
                    if (this.game.EditObj.mouseOverActive & this.game.EditObj.udsOrderPossible)
                    {
                      if (this.game.EditObj.SoundOn)
                        SoundMod.PlayAWave(this.game.AppPath + "sound/transfer.wav", ref this.game.EditObj);
                      this.game.EventRelatedObj.DoCheckSpecificEvent((int) Math.Round(Conversion.Val((object) this.game.Data.RuleVar[704])));
                      this.DrawLayersAndSuch();
                      this.game.EditObj.UnitSelected = -1;
                      this.game.EditObj.OrderData = -1;
                      this.game.EditObj.udsOrderPossible = false;
                      this.game.EditObj.OrderX = -1;
                      WindowReturnClass windowReturnClass7 = (WindowReturnClass) this.UdsClickUnit(this.game.SelectX, this.game.SelectY, mapSelected1, false);
                  ...
                  if (this.game.EditObj.PencilType == 1)
                  {
                    if (this.game.EditObj.PencilMode == 0)
                    {
                      ...
                      if (this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.SelectX, this.game.SelectY].Location > -1)
                        index2 = (int) Interaction.MsgBox((object) "There is a location in this hex. Are you sure to change landscape?", MsgBoxStyle.YesNo, (object) "Shadow Empire : Planetary Conquest");
                      if (index2 == 6)
                      {
                    ...
                    if (this.game.EditObj.OrderType == 1 | this.game.EditObj.OrderType == 48)
                    {
                      ...
                      for (int index12 = 0; index12 <= counter; ++index12)
                      {
                        ...
                        if (this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.SelectX, this.game.SelectY].UnitCounter > 14 & (this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.SelectX, this.game.SelectY].Regime == this.game.Data.Turn | this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.SelectX, this.game.SelectY].Regime == -1))
                        {
                          ...
                          this.game.SelectY = selectY1;
                          this.DrawLayersAndSuch();
                          WindowReturnClass windowReturnClass8 = (WindowReturnClass) this.UdsClickUnit(this.game.SelectX, this.game.SelectY, mapSelected1, false);
                        ...
                        else if (this.game.EditObj.TempValue[this.game.EditObj.MapSelected].Value[this.game.SelectX, this.game.SelectY] < 9999)
                        {
                          ...
                          if (orderResult3.BattleUnit == -1)
                          {
                            ...
                            {
                              if (orderResult3.OK)
                                this.game.EditObj.TempCoordListLastMove.AddList(ref orderResult3.CList);
                            }
              ...
              if (!this.game.Data.PermanentOverlayUse)
              {
                ...
                subPart1.PaintCoordinate((Graphics) null, x7, y7, map5, gBitmap: (ref local1));
                ...
                subPart2.PaintCoordinate((Graphics) null, selectX, selectY3, mapSelected2, gBitmap: (ref local2));
                if (this.game.Data.Round == 0 & this.game.EditObj.PencilType == 1)
                {
                  ...
                  do
                  {
                    ...
                    if (coordinate5.onmap)
                    {
                      ...
                      subPart3.PaintCoordinate((Graphics) null, x8, y8, map6, gBitmap: (ref local3));
                    }
                    ++tfacing;
                  }
                  while (tfacing <= 6);
    ...
    public bool DrawLayersAndSuch(bool noClearMap = false)
    {
      int num1 = 0;
      if (this.game.EditObj.OrderType == 26 | this.game.EditObj.AIMoving)
        return false;
      ...
      if (this.game.EditObj.ShowAirRange & this.game.EditObj.OrderType == 0)
      {
        ...
        if (this.game.EditObj.UnitSelected > -1 & this.game.EditObj.OldUnit != this.game.EditObj.UnitSelected && this.game.Data.Turn == this.game.Data.UnitObj[this.game.EditObj.UnitSelected].Regime)
        {
          if (this.game.HandyFunctionsObj.HasUnitAirSF(this.game.EditObj.UnitSelected))
          {
            int increaseap = (int) Math.Round((double) (Conversion.Int((float) this.game.HandyFunctionsObj.GetLowestAirRdn(this.game.EditObj.UnitSelected, true) * this.game.Data.RuleVar[147]) - (float) this.game.HandyFunctionsObj.GetLowestAirRdn(this.game.EditObj.UnitSelected, true)));
            this.game.HandyFunctionsObj.RedimTempValue(9999);
      ...
      if (this.game.EditObj.ShowLISRange & (double) this.game.Data.RuleVar[403] > 0.0)
      {
        ...
        if (flag2)
        {
          ...
          AIMatrix ownerMatrix = new AIMatrix(ref this.game.DC2AIObj);
          ...
          aiMatrix.ExpandAsSimplifiedSupplyMatrix(10, ref ownerMatrix, 1, (AICoordinateMatrix) null);
        ...
        if (this.game.EditObj.ShowSameHistorical)
        {
          if (this.game.EditObj.OldUnit > -1 && !this.game.Data.UnitObj[this.game.EditObj.OldUnit].IsHQ & this.game.Data.UnitObj[this.game.EditObj.OldUnit].Regime == this.game.Data.Turn & this.game.Data.UnitObj[this.game.EditObj.OldUnit].HQ > -1)
          {
            ...
            for (int index = 0; index <= unitCounter; ++index)
            {
              ...
              {
                SubPartClass subPart = this.SubPartList[0];
                int x = this.game.Data.UnitObj[index].X;
                int y = this.game.Data.UnitObj[index].Y;
                int map = num1;
                bitmap = (Bitmap) null;
                ref Bitmap local = ref bitmap;
                subPart.PaintCoordinate((Graphics) null, x, y, map, gBitmap: (ref local));
              }
            }
          }
    ...
    public bool CheckMovePath(int subpartMapSlot)
    {
          ...
          if (flag)
          {
            if (this.game.EditObj.MouseOverX > -1 & this.game.EditObj.MouseOverY > -1 && !(this.lastUnitSelected == this.game.EditObj.UnitSelected & this.lastMouseOverX == this.game.EditObj.MouseOverX & this.lastMouseOverY == this.game.EditObj.MouseOverY) | !this.game.EditObj.mouseOverActive)
            {
              ...
              if (flag)
              {
                ...
                {
                  this.game.EditObj.TempCoordList.AddList(ref this.game.EditObj.TempMovePathList);
                  int groupMoveCounter = this.game.EditObj.tempGroupMoveCounter;
                  for (int index = 0; index <= groupMoveCounter; ++index)
                  {
                    if (!Information.IsNothing((object) this.game.EditObj.tempGroupMovePath[index]))
                      this.game.EditObj.TempCoordList.AddList(ref this.game.EditObj.tempGroupMovePath[index]);
                  }
                }
                this.game.EditObj.TempMovePathList = new CoordList();
                ...
                if (this.game.Data.Product >= 6)
                {
                  ...
                  for (int index = 0; index <= groupMoveCounter; ++index)
                  {
                    this.game.EditObj.tempGroupMovePath[index] = new CoordList();
                ...
                if (this.game.EditObj.udsUnitOrderMode == 36)
                  this.game.EventRelatedObj.DoCheckSpecificEvent((int) Math.Round(Conversion.Val((object) this.game.Data.RuleVar[703])));
                return true;
              }
            }
          }
    ...
    public override WindowReturnClass HandleMouseMove(int x, int y)
    {
      ...
      if (this.SubPartCounter > -1)
      {
        int subPartCounter = this.SubPartCounter;
        bool t;
        for (int subpartMapSlot = 0; subpartMapSlot <= subPartCounter; ++subpartMapSlot)
        {
    ...
    public object EditorPlaceLocation()
    {
      ...
      {
        string str = Conversions.ToString(Conversion.Val(Interaction.InputBox("Give People # for town...", "Shadow Empire : Planetary Conquest")));
        if (Operators.CompareString(str, "", false) == 0)
          return (object) -1;
        num1 = (int) Math.Round(Conversion.Val(str));
