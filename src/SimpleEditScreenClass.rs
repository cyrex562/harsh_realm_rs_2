...
namespace WindowsApplication1
{
  pub class SimpleEditScreenClass : ScreenClass
  {
    ...
    pub SimpleEditScreenClass( tgame: GameClass, tformref: Form1)
      : base( tgame, tgame.BACKGROUND3MARC, tformref)
    {
      ...
      self.WEditMenu = self.AddWindow((WindowClass) new SimpleEditOptionsWindowClass( tgame, self.OwnBackground, 0, 0), 0, 0, self.Game.ScreenWidth, 50);
      self.Wmiddle = self.AddWindow((WindowClass) new SimpleEditDashboardWindowClass( tgame), 0, 50, self.Game.ScreenWidth, self.Game.ScreenHeight - 50);
    }
            ...
            for (let mut index2: i32 = 0; index2 <= counter; index2 += 1)
            {
              ...
              if (windowReturnClass.CommandType[index2] == 2)
              {
                ...
                if (windowReturnClass.CommandData[index2] == 109)
                  self.Wmiddle = self.AddWindow((WindowClass) new SimpleEditDebugWindowClass( DrawMod.TGame), 0, 50, self.Game.ScreenWidth, self.Game.ScreenHeight - 50);
                ...
                if (windowReturnClass.CommandData[index2] == 12)
                  self.wdown = self.AddWindow((WindowClass) new MapWindowClass2( DrawMod.TGame, 350, 0), 0, 350, self.Game.ScreenWidth, self.Game.ScreenHeight - 350);
              }
            }
          }
          screenReturnClass.flag = windowReturnClass.Flag;
