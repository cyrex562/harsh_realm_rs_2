...
{
  pub class SS_ScreenClass : ScreenClass
  {
     WEditMenu: i32;
     Wmiddle: i32;
     wdown: i32;
    ...
    pub SS_ScreenClass( tgame: GameClass, tformref: Form1)
      : base( tgame, tgame.BACKGROUND3MARC, tformref)
    {
      ...
      self.WEditMenu = self.AddWindow((WindowClass) new SSEditOptionsWindowClass( tgame, self.OwnBackground, 0, 0), 0, 0, self.Game.ScreenWidth, 50);
      self.Wmiddle = self.AddWindow((WindowClass) new SSMapDashboardWindowClass( tgame), 0, 50, self.Game.ScreenWidth, self.Game.ScreenHeight - 50);
    }
            ...
            for (let mut index2: i32 = 0; index2 <= counter; index2 += 1)
            {
              ...
              if (windowReturnClass.CommandType[index2] == 2)
              {
                if (windowReturnClass.CommandData[index2] == 94)
                  self.WEditMenu = self.AddWindow((WindowClass) new SSEditOptionsWindowClass( DrawMod.TGame, self.OwnBackground, 0, 0), 0, 0, self.Game.ScreenWidth, 50);
                if (windowReturnClass.CommandData[index2] == 95)
                  self.Wmiddle = self.AddWindow((WindowClass) new SSMapDashboardWindowClass( DrawMod.TGame), 0, 50, self.Game.ScreenWidth, self.Game.ScreenHeight - 50);
                if (windowReturnClass.CommandData[index2] == 122)
                  self.Wmiddle = self.AddWindow((WindowClass) new SSEventPicsClass( DrawMod.TGame), 0, 50, self.Game.ScreenWidth, self.Game.ScreenHeight - 50);
                if (windowReturnClass.CommandData[index2] == 123)
                  self.Wmiddle = self.AddWindow((WindowClass) new SSSmallGfxClass( DrawMod.TGame), 0, 50, self.Game.ScreenWidth, self.Game.ScreenHeight - 50);
