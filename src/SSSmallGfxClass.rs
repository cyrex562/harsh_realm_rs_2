...
{
  pub class SSSmallGfxClass : WindowClass
  {
     listId: i32;
     ListClass listObj;
     loadMapId: i32;
    ...
    pub SSSmallGfxClass( tGame: GameClass)
      : base( tGame, tGame.ScreenWidth, tGame.ScreenHeight - 50, 9, tDoBorders: 1, tHeaderString: "Small Gfx")
    {
      self.detailnr = -1;
      self.DoStuff();
    }
    ...
    pub fn DoStuff()
    {
      ...
      let mut num2: i32 = 60;
      tText: String = "Small Gfx are images that can exist in 3 size. But its okay if only their regular size exists.\r\nThey are used for example by Hex Perks. ";
      DrawMod.DrawTextColouredMarc( objgraphics, "Small Gfx", self.game.MarcFont1, num1 + 25, num2, Color.White);
      ...
      let mut y1: i32 = num2 + 90;
      let mut num3: i32 = y1;
      let mut x1: i32 = num1 + 25;
      self.listObj = ListClass::new();
      tlistselect1: i32;
      if (self.game.Data.SmallPicCounter > -1)
      {
        tlistselect1 = -1;
        let mut num4: i32 = -1;
        ...
        {
          let mut num5: i32 = 0;
          if (self.game.Data.SmallLibId[tdata].libSlot == 0)
            num5 = 1;
          if (num5 == 1)
          {
            num4 += 1;
            self.listObj.add(tdata.ToString() + ") " + self.game.Data.SmallPicName[tdata], tdata);
      ...
      else
      {
        ...
        font: Font =  null;
         local2: Font =  font;
        let mut tsubpart2: SubPartClass =  new ListSubPartClass(listObj, tlistsize, 400, tlistselect2, game, tHeader: "Small Gfx", tbackbitmap: ( local1), bbx: bbx, bby: bby, overruleFont: ( local2), overruleItemSize: 20);
        self.listId = self.AddSubPart( tsubpart2, x1, y1, 400, (15 + num6) * 20, 0);
      }
      let mut num7: i32 = num1 + 480;
      let mut num8: i32 = num3 + 40;
      let mut tsubpart3: SubPartClass =  new TextButtonPartClass("Add Small Gfx", 240, tBackbitmap: ( self.OwnBitmap), bbx: num7, bby: num8, usefont: self.game.MarcFont3, useshadow: true, tMarcStyle: true);
      self.addId = self.AddSubPart( tsubpart3, num7, num8, 240, 35, 1);
      if (self.detailnr <= -1)
        return;
      let mut num9: i32 = num8 + 50;
      let mut tsubpart4: SubPartClass =  new TextButtonPartClass("Remove Small Gfx", 240, tBackbitmap: ( self.OwnBitmap), bbx: num7, bby: num9, usefont: self.game.MarcFont3, useshadow: true, tMarcStyle: true);
      self.removeId = self.AddSubPart( tsubpart4, num7, num9, 240, 35, 1);
      let mut num10: i32 = num9 + 50;
      let mut tsubpart5: SubPartClass =  new TextButtonPartClass("Replace Small Gfx", 240, tBackbitmap: ( self.OwnBitmap), bbx: num7, bby: num10, usefont: self.game.MarcFont3, useshadow: true, tMarcStyle: true);
      self.loadId = self.AddSubPart( tsubpart5, num7, num10, 240, 35, 1);
      let mut num11: i32 = num10 + 50;
      let mut nr: i32 = self.game.Data.SmallPicNr[self.detailnr];
      ...
      bitmap: Bitmap = BitmapStore.GetBitmap(self.game.Data.SmallPicNr[self.detailnr]);
       let mut local4: &Bitmap = &bitmap;
      let mut x2: i32 = num7;
      let mut y2: i32 = num11;
      let mut w: i32 = num12;
      let mut h: i32 = num13;
            ...
            if (num1 != self.e1id)
            {
              ...
              if (num1 == self.addId)
              {
                ...
                if (File.Exists(self.game.AppPath + "graphics/" + filename))
                {
                  ...
                  self.detailnr = self.game.Data.SmallPicCounter;
                  self.game.Data.SmallLibId[self.detailnr].libSlot = 0;
                  let mut num3: i32 = 0;
                  let mut num4: i32 = self.detailnr - 1;
                  for (let mut index2: i32 = 0; index2 <= num4; index2 += 1)
                  {
                    if (self.game.Data.SmallLibId[index2].libSlot == 0 && self.game.Data.SmallLibId[index2].id > num3)
                      num3 = self.game.Data.SmallLibId[index2].id;
                  }
                  self.game.Data.SmallLibId[self.detailnr].id = num3 + 1;
                }
                else
                {
              ...
              {
                self.game.Data.RemoveSmallPic(self.detailnr);
                self.DoStuff();
                windowReturnClass.SetFlag(true);
                return windowReturnClass;
              }
              if (num1 == self.loadId)
              {
                ...
                if (File.Exists(self.game.AppPath + "graphics/" + filename))
                {
                  self.game.Data.SmallPicReplaceprite(self.detailnr, filename);
                }
                else
                {
