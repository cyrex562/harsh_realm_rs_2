          ...
          for (let mut index: i32 =  0; index <= counter; index += 1)
          {
            ...
            if (windowReturnClass2.CommandType[index] == 2)
            {
              ...
              if (windowReturnClass2.CommandData[index] == 50)
                this.wTop = this.AddWindow((WindowClass) new RandomWindowClass( this.Game),  Math.Round(Conversion.Int((double) (this.Game.ScreenWidth - 1024) / 2.0)),  Math.Round(Conversion.Int((double) (this.Game.ScreenHeight - 768) / 2.0)), 1024, 768);
            }
          }
        }
      }
