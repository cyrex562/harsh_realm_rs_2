...
{
  public class HistoryWindowClass : WindowClass
  {
    private int Info1Id;
    private int info2id;
    private int info3id;
    ...
    public HistoryWindowClass(ref GameClass tGame, Bitmap screenbitmap = null, int sx = -1, int sy = -1)
      : base(ref tGame, tGame.ScreenWidth, 250, 99, screenbitmap: screenbitmap, sx: sx, sy: sy)
    {
      this.dostuff();
      this.StartStep = -1;
      this.EndStep = 0;
    ...
    public void Forward(int steps)
    {
      ...
      if (num6 != 1)
        return;
      this.game.EditObj.TempCoordList = new CoordList();
      int num7 = (int) Math.Round((double) this.game.SelectX - (double) num4 / 2.0);
      int num8 = (int) Math.Round((double) this.game.SelectY - (double) num5 / 2.0);
      if (0 > num7)
        num7 = 0;
      if (0 > num8)
        num8 = 0;
      ...
      if (this.game.Data.Round == 0)
        num9 += 100;
      int num10 = (int) Math.Round((double) (this.game.ScreenWidth - 0) / 53.0);
      int num11 = (int) Math.Round((double) (this.game.ScreenHeight - num9) / 48.0);
    ...
    public void dostuffonlyslider()
    {
      ...
      if (this.StartStep > -1)
      {
        SubPartClass tsubpart = (SubPartClass) new NumberSliderSubPartClass2(this.game, "Step", " of " + Conversion.Str((object) Math.Max(this.Curstep, this.EndStep)), 340, 0, this.EndStep, this.Curstep, tbackbitmap: (ref this.OwnBitmap), bbx: (num1 + 5), bby: (num2 + 50));
        this.Slider1Id = this.AddSubPart(ref tsubpart, num1 + 5, num2 + 50, 340, 40, 0);
      }
      if (this.HumanPlayer == -1 | !this.game.AIThreadRunning)
      {
        SubPartClass tsubpart = (SubPartClass) new SteveButtonPartClass(this.game.BACKBUTTON, tBackbitmap: (ref this.OwnBitmap), bbx: (num1 + 5), bby: (num2 + 10));
        this.ExitId = this.AddSubPart(ref tsubpart, num1 + 5, num2 + 10, 35, 35, 1);
      }
      this.writing = false;
    }
    ...
    public void dostuff()
    {
      ...
      int num1 = (int) Math.Round((double) (this.game.ScreenWidth - 1024) / 2.0);
      int num2 = 5;
      DrawMod.DrawBlock(ref objGraphics, 0, 0, this.game.ScreenWidth, 40, (int) this.game.VicColor5.R, (int) this.game.VicColor5.G, (int) this.game.VicColor5.B, (int) this.game.VicColor5.A);
      DrawMod.DrawBlock(ref objGraphics, 0, 40, this.game.ScreenWidth, 10, (int) this.game.VicColor4.R, (int) this.game.VicColor4.G, (int) this.game.VicColor4.B, (int) this.game.VicColor4.A);
      DrawMod.drawLine(ref objGraphics, 0, 50, this.game.ScreenWidth, 50, (int) this.game.VicColor3.R, (int) this.game.VicColor3.G, (int) this.game.VicColor3.B, (int) this.game.VicColor3.A);
      this.game.EditObj.HumanPlayer = this.HumanPlayer;
      SubPartClass tsubpart1 = (SubPartClass) new MiniMapPartClass(this.game, ty: 130, ZoomLevel: 0, humanplayer: this.HumanPlayer);
      this.MiniMapId = this.AddSubPart(ref tsubpart1, num1 + 5, num2 + 102, 200, 130, 0);
      DrawMod.DrawRectangle(ref objGraphics, num1 + 4, num2 + 101, 201, 131, (int) DrawMod.TGame.VicColor3.R, (int) DrawMod.TGame.VicColor3.G, (int) DrawMod.TGame.VicColor3.B, (int) DrawMod.TGame.VicColor3.A);
      if (this.game.Data.MapCounter > 0)
      {
        ...
        tsubpart1 = (SubPartClass) new ListSubPartClass(mapListObj, 6, 180, tlistselect, game, tHeader: "Maps", tbackbitmap: (ref local1), bbx: 220, bby: 95, overruleFont: (ref local2));
        this.mapid = this.AddSubPart(ref tsubpart1, 220, 95, 120, 144, 0);
      }
      if (this.HumanPlayer == -1 | !this.game.AIThreadRunning)
      {
        tsubpart1 = (SubPartClass) new SteveButtonPartClass(this.game.BACKBUTTON, tBackbitmap: (ref this.OwnBitmap), bbx: 3, bby: 3);
        this.ExitId = this.AddSubPart(ref tsubpart1, 3, 3, 35, 35, 1);
      }
      if (this.game.AIRunning | this.game.AIThreadRunning)
      {
        tsubpart1 = (SubPartClass) new ATTextPartClass("AI " + this.game.Data.RegimeObj[this.game.Data.Turn].Name + " is thinking and moving", this.game.VicFont1, 350, 27, true, toutline: true);
        this.info2id = this.AddSubPart(ref tsubpart1, num1 + 350, 6, 325, 27, 0);
      }
      else if (!this.game.AIRunning & !this.game.AIThreadRunning & this.game.EditObj.AIMoving)
      {
        tsubpart1 = (SubPartClass) new ATTextPartClass("AI has finished its turn", this.game.VicFont1, 350, 27, true, toutline: true);
        this.info2id = this.AddSubPart(ref tsubpart1, num1 + 350, 6, 325, 27, 0);
      }
      else
      {
        tsubpart1 = (SubPartClass) new ATTextPartClass("History Screen", this.game.VicFont1, 350, 27, true);
        this.info2id = this.AddSubPart(ref tsubpart1, num1 + 350, 6, 325, 27, 0);
      }
      if (this.StartStep > -1)
      {
        tsubpart1 = (SubPartClass) new NumberSliderSubPartClass2(this.game, "Step", " of " + Conversion.Str((object) Math.Max(this.Curstep, this.EndStep)), 365, 0, this.EndStep, this.Curstep, tbackbitmap: (ref this.OwnBitmap), bbx: (num1 + 5), bby: (num2 + 50));
        this.Slider1Id = this.AddSubPart(ref tsubpart1, num1 + 5, num2 + 50, 365, 40, 0);
      }
      if (this.Curstep == 0)
      {
        tsubpart1 = (SubPartClass) new ATTextPartClass("Viewing History, Use Slider to view", this.game.VicFont2, 600, 192, true, tBlackBack: true);
        this.info3id = this.AddSubPart(ref tsubpart1, num1 + 400, num2 + 49, 600, 192, 0);
      }
      tsubpart1 = (SubPartClass) new TextButtonPartClass("Back", 150, "Click to start history again at step 0", ref this.OwnBitmap, num1 + 220, num2 + 132, theight: 30);
      this.Info1Id = this.AddSubPart(ref tsubpart1, num1 + 220, num2 + 132, 150, 30, 1);
      tsubpart1 = (SubPartClass) new TextButtonPartClass("Next Battle", 150, "Click to forward to the next step in history where a battle is reported", ref this.OwnBitmap, num1 + 220, num2 + 164, theight: 30);
      this.SpecialId = this.AddSubPart(ref tsubpart1, num1 + 220, num2 + 164, 150, 30, 1);
      tsubpart1 = (SubPartClass) new TextButtonPartClass("Next Regime", 150, "Click to forward to the next step in history caused by another regime", ref this.OwnBitmap, num1 + 220, num2 + 196, theight: 30);
      this.SkipId = this.AddSubPart(ref tsubpart1, num1 + 220, num2 + 196, 150, 30, 1);
      if (!this.AutoPlay)
      {
        tsubpart1 = (SubPartClass) new TextButtonPartClass("Play", 150, "Click to start Autoplay [shortkey P]", ref this.OwnBitmap, num1 + 220, num2 + 100, theight: 30);
        this.AutoPlayID = this.AddSubPart(ref tsubpart1, num1 + 220, num2 + 100, 150, 30, 1);
      }
      else
      {
        tsubpart1 = (SubPartClass) new TextButtonPartClass("Stop", 150, "Click to pause Autoplay [shortkey P]", ref this.OwnBitmap, num1 + 220, num2 + 100, theight: 30);
        this.AutoPlayID = this.AddSubPart(ref tsubpart1, num1 + 220, num2 + 100, 150, 30, 1);
      }
      if (this.game.EditObj.HisLossCounter > -1)
      {
        try
        {
          ...
          tsubpart1 = (SubPartClass) new ATListSubPartClass(this.OptionsListObj, 7, 290, -1, this.game, true, "ATTACKER: " + Strings.UCase(this.game.Data.RegimeObj[this.game.EditObj.HisLossAttReg].Name), tShowPair: true, tValueWidth: 140, tbackbitmap: (ref this.OwnBitmap), bbx: (num1 + 400), bby: (num2 + 90));
          this.OptionsListId = this.AddSubPart(ref tsubpart1, num1 + 400, num2 + 90, 290, 160, 0);
        }
        catch (Exception ex)
        {
        ...
        try
        {
          ...
          int hisLossCounter = this.game.EditObj.HisLossCounter;
          for (int index3 = 0; index3 <= hisLossCounter; ++index3)
          {
          ...
          {
            SubPartClass tsubpart2 = (SubPartClass) new ATListSubPartClass(this.OptionsList2Obj, 7, 290, -1, this.game, true, "DEFENDER: NEUTRAL?", tShowPair: true, tValueWidth: 140, tbackbitmap: (ref this.OwnBitmap), bbx: (num1 + 710), bby: (num2 + 90));
            this.OptionsList2Id = this.AddSubPart(ref tsubpart2, num1 + 710, num2 + 90, 290, 160, 0);
          }
          else
          {
            SubPartClass tsubpart3 = (SubPartClass) new ATListSubPartClass(this.OptionsList2Obj, 7, 290, -1, this.game, true, "DEFENDER: " + Strings.UCase(this.game.Data.RegimeObj[this.game.EditObj.HisLossDefReg].Name), tShowPair: true, tValueWidth: 140, tbackbitmap: (ref this.OwnBitmap), bbx: (num1 + 710), bby: (num2 + 90));
            this.OptionsList2Id = this.AddSubPart(ref tsubpart3, num1 + 710, num2 + 90, 290, 160, 0);
          }
        }
        catch (Exception ex)
        {
        ...
        {
          SubPartClass tsubpart4 = (SubPartClass) new ATTextPartClass(txt, this.game.VicFont1, 600, 27, true, tBlackBack: true);
          this.info5id = this.AddSubPart(ref tsubpart4, num1 + 400, num2 + 49, 600, 27, 0);
          if (Strings.Len(this.game.EditObj.HisInfoString) > 0)
          {
            SubPartClass tsubpart5 = (SubPartClass) new ATTextPartClass(this.game.EditObj.HisInfoString, this.game.VicFont3, 600, 12, true, tBlackBack: true);
            this.info6id = this.AddSubPart(ref tsubpart5, num1 + 400, num2 + 75, 600, 12, 0);
          }
        }
        else
        {
          SubPartClass tsubpart6 = (SubPartClass) new ATTextPartClass(this.game.EditObj.HisRegimeWon != this.game.EditObj.HisLossDefReg ? txt + " ATTACKER WAS VICTORIOUS" : txt + ": DEFENDER STOOD FIRM", this.game.VicFont1, 600, 27, true, tBlackBack: true);
          this.info5id = this.AddSubPart(ref tsubpart6, num1 + 400, num2 + 49, 600, 27, 0);
          if (Strings.Len(this.game.EditObj.HisInfoString) > 0)
          {
            SubPartClass tsubpart7 = (SubPartClass) new ATTextPartClass(this.game.EditObj.HisInfoString, this.game.VicFont3, 600, 12, true, tBlackBack: true);
            this.info6id = this.AddSubPart(ref tsubpart7, num1 + 400, num2 + 75, 600, 12, 0);
          }
        }
      }
      else if (Strings.Len(this.game.EditObj.HisInfoString) > 0)
      {
        tsubpart1 = (SubPartClass) new ATTextPartClass(this.game.EditObj.HisInfoString, this.game.VicFont1, 600, 192, true, tBlackBack: true);
        this.info5id = this.AddSubPart(ref tsubpart1, num1 + 400, num2 + 49, 600, 192, 0);
      }
              ...
              if (this.lastregime > -1)
              {
                ...
                while (num4 == 1)
                {
                  ...
                  if (this.Curstep < this.EndStep)
                  {
                    ...
                    {
                      int num5 = (int) Interaction.MsgBox((object) ("History from " + this.game.Data.RegimeObj[lastregime2].Name), Title: ((object) "Shadow Empire : Planetary Conquest"));
                      num4 = 0;
                    }
                  }
                }
