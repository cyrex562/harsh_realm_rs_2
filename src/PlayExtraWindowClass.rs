...
namespace WindowsApplication1
{
  pub class PlayExtraWindowClass : WindowClass
  {
    ...
    pub PlayExtraWindowClass( GameClass tGame, Bitmap screenbitmap = null, int sx = -1, int sy = -1)
    ...
    pub string WindowDescription(int x, int y)
    {
      ...
      int num =  Math.Round((double) (this.game.ScreenWidth - 1024) / 2.0);
      return str;
    }
    ...
    pub void dostuff()
    {
      ...
      this.ClearMouse();
      int h = 200;
      int x1 =  Math.Round((double) (this.game.ScreenWidth - 1024) / 2.0);
      int num2 = 10;
      ...
      if (num1 == 1 && this.game.EditObj.UnitSelected > -1 & this.game.EditObj.UnitSelected <= this.game.Data.UnitCounter && this.game.HandyFunctionsObj.CanWeSeeUnit(this.game.EditObj.UnitSelected, this.game.Data.Turn) > 0 & coordinate.x >= 2)
      {
        ...
        if (this.game.EditObj.SetViewMode2 == 0)
        {
          ...
          if (num9 == 1)
          {
            ...
            for (int tdata = 0; tdata <= sfCount; tdata += 1)
            {
      ...
      if (false)
      {
        ...
        int num19 =  Math.Round((double) this.game.ScreenWidth / 2.0 - 174.0);
        int y1 = 55;
