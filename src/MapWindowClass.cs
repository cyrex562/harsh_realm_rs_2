...
{
  public class MapWindowClass : WindowClass
  {
    private int MapId;
    ...
    public MapWindowClass(ref GameClass tGame, int tminheight = 0, int tminwidth = 200, int tZoomLevel = -2)
      : base(ref tGame, tGame.ScreenWidth - tminwidth, tGame.ScreenHeight - tminheight)
    {
      this.minheight = tminheight;
      this.minwidth = tminwidth;
      SubPartClass tsubpart = (SubPartClass) new MapPartClass(tGame.ScreenWidth - tminwidth, tGame.ScreenHeight - tminheight, tGame, ZoomLevel: tZoomLevel);
    ...
    public override WindowReturnClass handleTimer()
    {
      ...
      if (!this.game.AIThreadRunning && this.game.EditObj.BattleTimerActive)
      {
        if (this.game.EditObj.BattleAnimNr == 0 && this.game.EditObj.SoundOn)
          SoundMod.PlayAWave(this.game.AppPath + "sound/att_arm3.wav", ref this.game.EditObj);
        if (DateTime.Compare(DateTime.Now, this.game.EditObj.BattleTimer) > 0)
        {
          ...
          if (this.game.TempCombat.Init(new Coordinate()
          {
        ...
        subPart.PaintCoordinate((Graphics) null, targetX, targetY, mapSelected, gBitmap: (ref local));
    ...
    public override WindowReturnClass HandleKeyPress(int nr, bool fromTimer = false)
    {
      ...
      if (nr == 46)
      {
        ...
        int num = (int) Interaction.MsgBox((object) ("Memory Used Right Now by GUI (" + Conversion.Str((object) memorySize1) + "KB) + Bitmapstore (" + Conversion.Str((object) memorySize2) + "KB) =" + Conversion.Str((object) (int) Math.Round((double) (memorySize1 + memorySize2) / 1000.0)) + "MB. (Will write log files after you press OK)"), Title: ((object) "Shadow Empire : Planetary Conquest"));
        ...
        for (int nr1 = 0; nr1 <= counter1; ++nr1)
        {
      ...
      if (this.game.Data.Round == 0 & this.game.SelectX > -1)
      {
        ...
        {
          case 33:
            new Form3((Form) this.formref).Initialize(this.game.Data, 87, this.game.SelectX, this.game.SelectY, DrawMod.TGame);
            windowReturnClass.SetFlag(true);
            return windowReturnClass;
          case 35:
            if (this.game.SelectX > -1)
            {
              this.game.Data.MapObj[0].HexObj[this.game.SelectX, this.game.SelectY].VP = (int) Math.Round(Conversion.Val(Interaction.InputBox("Give new VP for hex", "Shadow Empire : Planetary Conquest")));
              break;
            }
            break;
          case 36:
            new Form3((Form) this.formref).Initialize(this.game.Data, 47, this.game.SelectX, this.game.SelectY, DrawMod.TGame);
            windowReturnClass.SetFlag(true);
            return windowReturnClass;
        }
      }
      if (this.game.Data.Round == 0 & this.game.EditObj.UnitSelected > -1)
      {
        if (nr == 55)
        {
          new Form3((Form) this.formref).Initialize(this.game.Data, 37, this.game.EditObj.UnitSelected);
          windowReturnClass.SetFlag(true);
          return windowReturnClass;
        }
        if (nr == 48 & this.game.Data.UnitObj[this.game.EditObj.UnitSelected].Historical > -1)
        {
          new Form3((Form) this.formref).Initialize(this.game.Data, 36, this.game.Data.UnitObj[this.game.EditObj.UnitSelected].Historical);
          windowReturnClass.SetFlag(true);
          return windowReturnClass;
        }
        ...
        {
          this.game.Data.HistoricalUnitObj[this.game.Data.UnitObj[this.game.EditObj.UnitSelected].Historical].Counter = (int) Math.Round(Conversion.Val(Interaction.InputBox("Give NATO counter # for this historical unit. (see systemgraphics/nato directory for numbers and pictures)")));
          this.SubPartList[this.SubpartNr(this.MapId)].Paint();
          windowReturnClass.SetFlag(true);
          return windowReturnClass;
        }
        if (nr == 45 & this.game.Data.UnitObj[this.game.EditObj.UnitSelected].Historical == -1)
        {
          ...
          else
          {
            ...
            this.game.Data.HistoricalUnitObj[historicalUnitCounter].Counter = (int) Math.Round(Conversion.Val(Interaction.InputBox("NATO counter #")));
            ...
            {
              int Number = (int) Math.Round(Conversion.Val(Strings.Mid(this.game.Data.UnitObj[unitSelected].Name, Start)));
              if (Number > 0)
              {
      ...
      int num1 = 0;
      int num2 = (int) Math.Round(Conversion.Int((double) this.OwnBitmap.Width / (double) (30 * (this.game.EditObj.Zoom + 2))));
      int num3 = this.game.EditObj.Zoom != 1 ? (int) Math.Round(Conversion.Int((double) (this.OwnBitmap.Height - num1) / (double) (24 * (this.game.EditObj.Zoom + 2)))) : (int) Math.Round(Conversion.Int((double) (this.OwnBitmap.Height - num1) / (double) (24 * (this.game.EditObj.Zoom + 3))));
    ...
    public override void DoRefresh() => this.SubPartList[this.SubpartNr(this.MapId)].Paint();
...
label_329:
        for (int index1 = 0; index1 <= subPartCounter; ++index1)
        {
          if (x > this.SubPartX[index1] & x < this.SubPartX[index1] + this.SubPartW[index1] && y > this.SubPartY[index1] & y < this.SubPartY[index1] + this.SubPartH[index1] && this.SubPartID[index1] == this.MapId)
          {
            ...
            if (coordinate1.onmap)
            {
              ...
              if (b == 1)
              {
                  ...
                  if (this.game.EditObj.PencilType == 1)
                  {
                    if (this.game.EditObj.PencilMode == 0)
                    {
                      ...
                      if (this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.SelectX, this.game.SelectY].Location > -1)
                        num1 = (int) Interaction.MsgBox((object) "There is a location in this hex. Are you sure to change landscape?", MsgBoxStyle.YesNo, (object) "Shadow Empire : Planetary Conquest");
                      if (num1 == 6)
                      {
                ...
                if (this.game.Data.Round > 0)
                {
                  ...
                  else if (this.game.EditObj.OrderType == 43)
                  {
                    ...
                    {
                      new Form3((Form) this.formref).Initialize(this.game.Data, 41, 0, 0, this.game);
                      this.game.EditObj.OrderType = 0;
                      this.game.EditObj.TempCoordList = new CoordList();
                      windowReturnClass.AddCommand(4, 12);
                            ...
                            if (lowestSpeed1 > -1 & index10 == 0)
                            {
                              if (this.game.Data.SFObj[lowestSpeed2].MoveType == -1)
                              {
                                if (Strings.Len(this.game.Data.SFTypeObj[lowestSpeed1].MoveWAV) > 0)
                                  SoundMod.PlayAWave(this.game.AppPath + "sound/" + this.game.Data.SFTypeObj[lowestSpeed1].MoveWAV, ref this.game.EditObj);
                              }
                              else if (Strings.Len(this.game.Data.TempString[900 + this.game.Data.SFObj[lowestSpeed2].MoveType]) > 0)
                                SoundMod.PlayAWave(this.game.AppPath + "sound/" + this.game.Data.TempString[900 + this.game.Data.SFObj[lowestSpeed2].MoveType], ref this.game.EditObj);
                            }
              ...
              {
                SubPartClass subPart1 = this.SubPartList[index1];
                int x5 = num1;
                int y5 = selectY1;
                int map5 = mapSelected1;
                Bitmap bitmap1 = (Bitmap) null;
                ref Bitmap local1 = ref bitmap1;
                subPart1.PaintCoordinate((Graphics) null, x5, y5, map5, gBitmap: (ref local1));
                Bitmap bitmap2;
                if (this.game.Data.Round == 0)
                {
                  SubPartClass subPart2 = this.SubPartList[index1];
                  int selectX = this.game.SelectX;
                  int selectY2 = this.game.SelectY;
                  int mapSelected2 = this.game.EditObj.MapSelected;
                  Bitmap bitmap3 = (Bitmap) null;
                  ref Bitmap local2 = ref bitmap3;
                  subPart2.PaintCoordinate((Graphics) null, selectX, selectY2, mapSelected2, gBitmap: (ref local2));
                  int tfacing = 1;
                  do
                  {
                    ...
                    {
                      SubPartClass subPart3 = this.SubPartList[index1];
                      int x6 = coordinate4.x;
                      int y6 = coordinate4.y;
                      int mapSelected3 = this.game.EditObj.MapSelected;
                      bitmap2 = (Bitmap) null;
                      ref Bitmap local3 = ref bitmap2;
                      subPart3.PaintCoordinate((Graphics) null, x6, y6, mapSelected3, gBitmap: (ref local3));
                    }
                    ++tfacing;
                  }
                  while (tfacing <= 6);
                ...
                {
                  SubPartClass subPart4 = this.SubPartList[index1];
                  int selectX = this.game.SelectX;
                  ...
                  subPart4.PaintCoordinate((Graphics) null, selectX, selectY3, mapSelected4, gBitmap: (ref local4));
                }
                if (this.game.Data.Round == 0 & this.game.EditObj.PencilType == 1)
                {
                  ...
                  do
                  {
                    ...
                    {
                      SubPartClass subPart5 = this.SubPartList[index1];
                      int x7 = coordinate5.x;
                      int y7 = coordinate5.y;
                      int map6 = coordinate5.map;
                      bitmap2 = (Bitmap) null;
                      ref Bitmap local5 = ref bitmap2;
                      subPart5.PaintCoordinate((Graphics) null, x7, y7, map6, gBitmap: (ref local5));
                    }
                    ++tfacing;
                  }
                  while (tfacing <= 6);
          ...
          if (x > this.SubPartX[index1] & x < this.SubPartX[index1] + this.SubPartW[index1] && y > this.SubPartY[index1] & y < this.SubPartY[index1] + this.SubPartH[index1] && this.SubPartID[index1] == this.MapId)
          {
            ...
            if (coordinate.onmap)
            {
              ...
              if (this.game.EditObj.OrderType == 0)
              {
                if (!this.game.EditObj.LayerSupplyOn)
                {
                  string str1 = this.game.HandyFunctionsObj.GetHexName(coordinate.x, coordinate.y, coordinate.map) + "(" + Strings.Trim(Conversion.Str((object) coordinate.x)) + "," + Strings.Trim(Conversion.Str((object) coordinate.y)) + ")";
                  string str2 = this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[coordinate.x, coordinate.y].Regime != -1 ? str1 + " - " + this.game.Data.RegimeObj[this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[coordinate.x, coordinate.y].Regime].Name : str1 + " - Unoccupied Territory";
                ...
                {
                  string str3 = Strings.Trim(Conversion.Str((object) this.game.EditObj.TempSup[coordinate.map].Value[coordinate.x, coordinate.y])) + "ap/" + Strings.Trim(Conversion.Str((object) this.game.Data.RuleVar[3]));
                  string str4 = "";
                  ...
                  if (num > 0)
                  {
                    ...
                    if (this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.EditObj.OrderX, this.game.EditObj.OrderY].RiverType[index2] > -1)
                    {
                      if (this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.EditObj.OrderX, this.game.EditObj.OrderY].Bridge[index2])
                      {
                        ...
                        int Number2 = (int) Math.Round((double) Conversion.Int(this.game.Data.RuleVar[7] * this.game.Data.RiverTypeObj[this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.EditObj.OrderX, this.game.EditObj.OrderY].RiverType[index2]].BridgeCostModifier));
                  ...
                  if (num > -1)
                  {
                    ...
                    if (x3 <= this.game.HandyFunctionsObj.GetLowestAp(this.game.EditObj.OrderUnit))
                    {
                      ...
                      if ((double) this.game.Data.RuleVar[32] == -1.0)
                      {
                        ...
                        {
                          if ((double) this.game.Data.RuleVar[822] == -1.0 | (double) this.game.Data.RegimeObj[this.game.Data.Turn].RegimeSlot[(int) Math.Round((double) Math.Max(0.0f, this.game.Data.RuleVar[822]))] >= (double) this.game.Data.RuleVar[823] + (double) this.game.Data.RiverTypeObj[this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.EditObj.OrderX, this.game.EditObj.OrderY].RiverType[facing]].BridgeCostModifier * (double) this.game.Data.RuleVar[825])
                          {
                      ...
                      {
                        if ((double) this.game.Data.RuleVar[822] == -1.0 | (double) this.game.Data.RegimeObj[this.game.Data.Turn].RegimeSlot[(int) Math.Round((double) Math.Max(0.0f, this.game.Data.RuleVar[822]))] >= (double) this.game.Data.RuleVar[823])
                        {
                          if ((double) this.game.Data.RoadTypeObj[(int) Math.Round((double) this.game.Data.RuleVar[32])].EPCost * (double) this.game.Data.LandscapeTypeObj[this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[coordinate.x, coordinate.y].LandscapeType].RoadCostModifier <= (double) this.game.HandyFunctionsObj.GetUnitEP(this.game.EditObj.OrderUnit))
                          {
                            str6 = "Road Cost: " + Conversion.Str((object) (float) ((double) this.game.Data.RoadTypeObj[(int) Math.Round((double) this.game.Data.RuleVar[32])].EPCost * (double) this.game.Data.LandscapeTypeObj[this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[coordinate.x, coordinate.y].LandscapeType].RoadCostModifier)) + " EP";
                            if ((double) this.game.Data.RuleVar[822] > -1.0)
                              str6 = str6 + ". " + Conversion.Str((object) this.game.Data.RuleVar[823]) + "x " + this.game.Data.RegimeSlotName[(int) Math.Round((double) this.game.Data.RuleVar[822])];
                          }
                          else
                            str6 = "Not enough EP. Road Cost: " + Conversion.Str((object) (float) ((double) this.game.Data.RoadTypeObj[(int) Math.Round((double) this.game.Data.RuleVar[32])].EPCost * (double) this.game.Data.LandscapeTypeObj[this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[coordinate.x, coordinate.y].LandscapeType].RoadCostModifier)) + " EP";
                        }
                        else
                        {
                      ...
                      {
                        if ((double) this.game.Data.RuleVar[822] == -1.0 | (double) this.game.Data.RegimeObj[this.game.Data.Turn].RegimeSlot[(int) Math.Round((double) Math.Max(0.0f, this.game.Data.RuleVar[822]))] >= (double) this.game.Data.RuleVar[824])
                        {
                          if ((double) this.game.Data.RoadTypeObj[(int) Math.Round((double) this.game.Data.RuleVar[821])].EPCost * (double) this.game.Data.LandscapeTypeObj[this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[coordinate.x, coordinate.y].LandscapeType].RoadCostModifier <= (double) this.game.HandyFunctionsObj.GetUnitEP(this.game.EditObj.OrderUnit))
                          {
                            str6 = "Road Cost: " + Conversion.Str((object) (float) ((double) this.game.Data.RoadTypeObj[(int) Math.Round((double) this.game.Data.RuleVar[821])].EPCost * (double) this.game.Data.LandscapeTypeObj[this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[coordinate.x, coordinate.y].LandscapeType].RoadCostModifier)) + " EP";
                            if ((double) this.game.Data.RuleVar[822] > -1.0)
                              str6 = str6 + ". " + Conversion.Str((object) this.game.Data.RuleVar[823]) + "x " + this.game.Data.RegimeSlotName[(int) Math.Round((double) this.game.Data.RuleVar[822])];
                          }
                          else
                            str6 = "Not enough EP. Road Cost: " + Conversion.Str((object) (float) ((double) this.game.Data.RoadTypeObj[(int) Math.Round((double) this.game.Data.RuleVar[821])].EPCost * (double) this.game.Data.LandscapeTypeObj[this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[coordinate.x, coordinate.y].LandscapeType].RoadCostModifier)) + " EP";
                        }
                        else
                        {
                          str6 = "Not enough resources to construct road.";
    ...
    public object EditorPlaceLocation()
    {
      int num1;
      if (this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.SelectX, this.game.SelectY].Regime == -1)
      {
        if (this.game.Data.PeopleCounter > 0)
        {
          ...
          string str2 = Conversions.ToString(Conversion.Val(Interaction.InputBox("Give People # for town..." + str1, "Shadow Empire : Planetary Conquest")));
          if (Operators.CompareString(str2, "", false) == 0)
            return (object) -1;
          num1 = (int) Math.Round(Conversion.Val(str2));
          if (num1 == -1 | num1 > this.game.Data.PeopleCounter)
          {
            int num2 = (int) Interaction.MsgBox((object) "Invalid people#", Title: ((object) "Shadow Empire : Planetary Conquest"));
            return (object) -1;
          }
        }
        else
      ...
      string Left = Interaction.InputBox("Give Name for this location...", "Shadow Empire : Planetary Conquest", this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.SelectX, this.game.SelectY].Name);
    ...
    public object regimeFill(int newreg)
    {
    ...
    public object areacodeFill(int slot, int code)
    {
      object[,] objArray = new object[this.game.Data.MapObj[this.game.EditObj.MapSelected].MapWidth + 1, this.game.Data.MapObj[this.game.EditObj.MapSelected].MapHeight + 1];
      objArray[this.game.SelectX, this.game.SelectY] = (object) 1;
    ...
    public object hexLibVarFill(int slot, int code)
    {
      object[,] objArray = new object[this.game.Data.MapObj[this.game.EditObj.MapSelected].MapWidth + 1, this.game.Data.MapObj[this.game.EditObj.MapSelected].MapHeight + 1];
      objArray[this.game.SelectX, this.game.SelectY] = (object) 1;
    ...
    public object landscapeFill(int newland, int newspr)
    {
      object[,] objArray = new object[this.game.Data.MapObj[this.game.EditObj.MapSelected].MapWidth + 1, this.game.Data.MapObj[this.game.EditObj.MapSelected].MapHeight + 1];
      objArray[this.game.SelectX, this.game.SelectY] = (object) 1;
    ...
    public object heightLevelFill(int newHeight)
    {
      object[,] objArray = new object[this.game.Data.MapObj[this.game.EditObj.MapSelected].MapWidth + 1, this.game.Data.MapObj[this.game.EditObj.MapSelected].MapHeight + 1];
      objArray[this.game.SelectX, this.game.SelectY] = (object) 1;
      int landscapeType = this.game.Data.MapObj[this.game.EditObj.MapSelected].HexObj[this.game.SelectX, this.game.SelectY].LandscapeType;
    ...
    public object specialFill(int newland, int newspr)
    {
      object[,] objArray = new object[this.game.Data.MapObj[this.game.EditObj.MapSelected].MapWidth + 1, this.game.Data.MapObj[this.game.EditObj.MapSelected].MapHeight + 1];
      objArray[this.game.SelectX, this.game.SelectY] = (object) 1;
