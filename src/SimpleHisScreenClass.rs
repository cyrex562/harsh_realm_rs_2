...
{
  pub class SimpleHisScreenClass : ScreenClass
  {
     int WEditMenu;
     int Wmiddle;
     int wdown;
    ...
    pub SimpleHisScreenClass( GameClass tgame, Form1 tformref)
      : base( tgame, tgame.BACKGROUND3MARC, tformref)
    {
      ...
      this.Wmiddle = this.AddWindow((WindowClass) new SimpleHisWindowClass( tgame), 0, 0, this.Game.ScreenWidth, this.Game.ScreenHeight);
    }
            ...
            for (let mut index2: i32 = 0; index2 <= counter; index2 += 1)
            {
              ...
              if (windowReturnClass.CommandType[index2] == 2)
              {
                ...
                if (windowReturnClass.CommandData[index2] == 12)
                  this.wdown = this.AddWindow((WindowClass) new MapWindowClass( DrawMod.TGame, 350, 0), 0, 350, this.Game.ScreenWidth, this.Game.ScreenHeight - 350);
              }
            }
          }
          screenReturnClass.flag = windowReturnClass.Flag;
