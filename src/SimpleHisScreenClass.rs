...
namespace WindowsApplication1
{
  pub class SimpleHisScreenClass : ScreenClass
  {
    ...
    pub SimpleHisScreenClass( tgame: GameClass, tformref: Form1)
      : base( tgame, tgame.BACKGROUND3MARC, tformref)
    {
      self.OffSetX =  Math.Round( (self.Game.ScreenWidth - 1024) / 2.0);
      self.AllowRightMouse = true;
      self.Game.EditObj.SimpleEditWindow = 104;
      self.Wmiddle = self.AddWindow((WindowClass) new SimpleHisWindowClass( tgame), 0, 0, self.Game.ScreenWidth, self.Game.ScreenHeight);
    }
