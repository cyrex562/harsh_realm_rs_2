...
{
  public class SFScreenClass : ScreenClass
  {
    private int wtop;

    public SFScreenClass(ref GameClass tGame, Form1 tformref)
      : base(ref tGame, tGame.BACKGROUND1MARC, tformref)
    {
      if (this.Game.EditObj.OrderType == 23 | this.Game.Data.CampaignRoom > -1)
      {
        if (this.Game.Data.CampaignRoom > -1)
          this.wtop = this.AddWindow((WindowClass) new OldResearchWindowClass(ref tGame, 0), (int) Math.Round((double) this.Game.ScreenWidth / 2.0 - 500.0), (int) Math.Round((double) this.Game.ScreenHeight / 2.0 - 375.0), 1024, 768);
        else
          this.wtop = this.AddWindow((WindowClass) new OldResearchWindowClass(ref tGame), (int) Math.Round((double) this.Game.ScreenWidth / 2.0 - 500.0), (int) Math.Round((double) this.Game.ScreenHeight / 2.0 - 375.0), 1024, 768);
      }
      else if (this.Game.EditObj.OrderType == 47)
        this.wtop = this.AddWindow((WindowClass) new ModelDesignerWindowClass(ref tGame), (int) Math.Round((double) (this.Game.ScreenWidth - 1024) / 2.0), (int) Math.Round((double) (this.Game.ScreenHeight - 768) / 2.0), 1024, 768);
      else
        this.wtop = this.AddWindow((WindowClass) new SFWindowClass(ref tGame), (int) Math.Round((double) (this.Game.ScreenWidth - 1024) / 2.0), (int) Math.Round((double) (this.Game.ScreenHeight - 768) / 2.0), 1024, 768);
    }

    public SFScreenClass(ref GameClass tGame, Form1 tformref, bool Marc)
      ...
      if (this.Game.EditObj.OrderType == 23 | this.Game.Data.CampaignRoom > -1)
      {
        if (this.Game.Data.CampaignRoom > -1)
        {
          if ((double) this.Game.Data.RuleVar[839] == 1.0)
            ...
            this.wtop = this.AddWindow((WindowClass) new PlayResearchWindowClass(ref tGame, 0), x, y, 1024, 768);
        }
        else
          this.wtop = this.AddWindow((WindowClass) new PlayResearchWindowClass(ref tGame), x, y, 1024, 768);
      }
      else if (this.Game.EditObj.OrderType == 47)
        this.wtop = this.AddWindow((WindowClass) new ModelDesignerWindowClass(ref tGame), (int) Math.Round((double) (this.Game.ScreenWidth - 1024) / 2.0), (int) Math.Round((double) (this.Game.ScreenHeight - 768) / 2.0), 1024, 768);
      else
