...
{
  public class SimpleMapScreenClass : ScreenClass
  {
    ...
    public SimpleMapScreenClass(ref GameClass tgame, Form1 tformref)
      : base(ref tgame, tgame.BACKGROUND3MARC, tformref)
    {
      ...
      this.WEditMenu = this.AddWindow((WindowClass) new SimpleEditMapOptionsWindowClass(ref tgame, this.OwnBackground, 0, 0), 0, 0, this.Game.ScreenWidth, 50);
      this.Wmiddle = this.AddWindow((WindowClass) new SimpleMapDashboardWindowClass(ref tgame), 0, 50, this.Game.ScreenWidth, this.Game.ScreenHeight - 50);
    }
            ...
            for (int index2 = 0; index2 <= counter; ++index2)
            {
              ...
              if (windowReturnClass.CommandType[index2] == 2)
              {
                if (windowReturnClass.CommandData[index2] == 108)
                  this.WEditMenu = this.AddWindow((WindowClass) new SimpleEditMapOptionsWindowClass(ref DrawMod.TGame, this.OwnBackground, 0, 0), 0, 0, this.Game.ScreenWidth, 50);
                if (windowReturnClass.CommandData[index2] == 107)
                  this.Wmiddle = this.AddWindow((WindowClass) new SimpleMapDashboardWindowClass(ref DrawMod.TGame), 0, 50, this.Game.ScreenWidth, this.Game.ScreenHeight - 50);
                if (windowReturnClass.CommandData[index2] == 112)
                  this.Wmiddle = this.AddWindow((WindowClass) new SimpleMapReplacementsWindowClass(ref DrawMod.TGame), 0, 50, this.Game.ScreenWidth, this.Game.ScreenHeight - 50);
                ...
                if (windowReturnClass.CommandData[index2] == 12)
                  this.wdown = this.AddWindow((WindowClass) new MapWindowClass(ref DrawMod.TGame, 350, 0), 0, 350, this.Game.ScreenWidth, this.Game.ScreenHeight - 350);
              }
            }
          }
          screenReturnClass.flag = windowReturnClass.Flag;
