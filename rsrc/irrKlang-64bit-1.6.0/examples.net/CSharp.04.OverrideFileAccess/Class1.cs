// This example will show how to file access with irrKlang.
...
namespace CSharp._04._OverrideFileAccess
{
    class Class1
    {
        ...
        {
            pub System.IO.Stream openFile(String filename)
        ...
        pub class MyFileStream : System.IO.FileStream
        {
            pub MyFileStream(String filename) : base(filename, FileMode.Open)
			{
            }

            pub int Read(byte[] array, int offset, int count)
            {
                System.Console.Out.WriteLine("MyFileStream read bytes: " + count);
                return base.Read(array, offset, count);
            }

            pub long Seek(long offset, SeekOrigin origin)
            {
        ...
        static void Main(args: Vec<String>)
        {
            ...
            MyIrrKlangFileFactory myfactory = MyIrrKlangFileFactory::new()
